...
| Recent messages | `GET /v1/inbucket/messages?scope=recent` with the same pagination, filter, and sort parameters | `200` available summaries across active mailboxes with per-user state, exact filtered total, and next cursor; no upstream refresh | `401` expired, `422` invalid query, range, tag, or cursor |
