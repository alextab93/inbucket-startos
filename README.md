...
| -------------- | ------ |
| Server state | Session, active and archived mailbox catalogs, user-owned tag definitions and assignments, message summaries with per-user starred state, all starred summaries, selected parsed message, attachments, monitor summaries |
| URL state | Active view, selected mailbox, and selected message identity in the `view`, `mailbox`, and `message` query parameters |
| User-input state | Credentials, selected mailboxes, search text, read filter, starred-view mailbox filter, sort, open tools, source visibility |
| Derived render values | Visible and ordered messages, counts, labels, empty explanations, selected styling, enabled actions |

The client remains on `/` and does not require a path-based SPA fallback. Mailboxes is the default view and omits `view`. Starred, Monitor, and Archived use `view=starred`, `view=monitor`, and `view=archive`. Mailboxes may use `mailbox` for one selected mailbox and add `message` for its selected message. Starred includes `mailbox` and `message` only for its own selected message. Monitor and Archived discard those selection parameters. View changes push browser history, direct loading and browser back or forward restore the represented view, and malformed combinations are replaced with a safe canonical URL.
...
- Navigation uses ordinary buttons with `aria-current`, not partial tab semantics. Each view updates the canonical query URL, reload and browser history restore that view, and a stale message selection cannot reopen from Monitor or Archived. Mailboxes, Monitor, and Archived retain their user controls while hidden. Monitor polling runs only while its authenticated view is visible.
...
- Monitor has loading, empty, populated, refreshed, unauthorized, and upstream-error outcomes. Its search, read filters, date range, All tags filter, sorting, and mailbox-message navigation use current upstream `seen` data and the signed-in user's assignments. Rails applies its date range before the 200-result limit. Its delivery list scrolls inside the viewport-height panel without moving the whole page on larger screens. Archived has dedicated loading, empty, populated, partial-count, catalog-error, restore-pending, and restore-error outcomes. Restore is non-destructive, while mailbox purge remains a separate confirmed action.
...
| Monitor summaries | `GET /v1/inbucket/monitor/messages` with optional ISO 8601 `received_after` and exclusive `received_before` | `200` filtered array capped by Rails at 200 | `401` expired, `422` invalid range, other status retryable |
...
| Starred messages | `GET /v1/inbucket/starred/messages` with optional ISO 8601 `received_after` and exclusive `received_before` | `200` current user's filtered stored message-summary array | `401` expired, `422` invalid range, other status visible load error |
...
2. Switch among Mailboxes, Starred, Monitor, and Archived, then use direct loading, reload, and browser back or forward to restore the view and any supported selected message from its canonical query URL.
3. Select one or several saved mailboxes and see one deterministic combined list. Add and open an archived name, archive selected names, confirm a purge, and retain failed names after partial results.
...
6. Render sanitized HTML, plaintext, CID and safe data images, safe external links, blocked unsafe content, and remote images only after consent inside the isolated frame.
7. Load and refresh Monitor only while visible, preserve its controls across refreshes, and open an arrival in its mailbox. Show Archived loading, empty, partial-count, catalog-error, restore-success, and restore-error results.
8. Complete the representative workflows with keyboard access, focus return, labels, headings, live announcements, and desktop, tablet, and mobile layouts.
