...
| -------------- | ------ |
| Server state | Session, active and archived mailbox catalogs, user-owned tag definitions and assignments, message summaries with per-user starred and trashed state, bounded live changes, all starred and trashed summaries, selected parsed message, attachments |
| URL state | Active view, selected mailboxes, and selected message identity in the `view`, repeated `mailboxes`, `mailbox`, and `message` query parameters |
...
Trash is per-user metadata linked to the same bounded `InbucketMessage` row with a `trashed_at` timestamp. Moving a message to Trash does not call upstream Inbucket, change upstream `seen`, remove a star, or copy its body, raw source, or attachments. The signed-in user's trashed messages are hidden from Mailboxes, Starred, and live additions while remaining visible to other users. Restore removes only that user's Trash link. Permanent deletion calls upstream first and removes the shared summary, monitor observation metadata, every user's stars and Trash links, and all tag assignments only after upstream succeeds. A missing upstream message remains visible in Trash as unavailable until the existing seven-day tombstone cleanup removes it.
...
- Trash lists the signed-in user's trashed messages across mailboxes with search, read state, mailbox filtering, and newest, oldest, largest, and smallest sorting. Its compact Empty trash action shares the list toolbar, avoiding a duplicate visible Trash heading. It has loading, empty, unavailable-message, and paginated states. Restore returns an available message to Mailboxes and Starred without removing its star. Delete permanently requires confirmation. Empty trash requires confirmation, reports one visible outcome per message, and leaves failures in Trash for a safe retry.
- Opening a message shows loading, not-found, upstream-error, and parsed-message states. Subject, From, To, Date, sanitized HTML or plaintext, attachment download cards with type and size, source, star control, tags, and deletion remain visible user behaviors. The Tag action shows current assignments, supports creation and management, closes with Escape or outside interaction, and restores focus. Tag assignment waits for persistence before changing visible badges, so a failed request preserves the exact prior list, inspector, selection, filter, and error state. Star changes update loaded mailbox views optimistically and roll back with a visible error if persistence fails. Read styling changes only after the read response succeeds. Changing selection prevents stale message, attachment, source, or renderer results from replacing the current message.
...
- Live Mailboxes preserves loaded pages, active filters and sorting, selection, scroll position, and keyboard focus as arrivals and deletions merge into the list. A selected message that becomes unavailable keeps its identity and shows the existing not-found outcome. Trash fills the remaining workspace height on desktop, tablet, and mobile. Archived has a responsive full-width card grid with a total count plus dedicated loading, empty, populated, partial-count, catalog-error, restore, and delete outcomes. Restore is non-destructive, while Delete mailbox permanently purges the archived mailbox after confirmation.
- Saved mailboxes and filter controls close on outside interaction. Escape or each panel's visible close button closes it and restores focus to the invoking control. The active mailbox panel owns the toolbar stacking layer so sibling triggers cannot cover it. The application preserves the skip link, headings, labels, live status regions, visible focus, destructive confirmation, and desktop, tablet, and mobile layouts. The phone layout keeps all four mailbox views visible, keeps the single Sign out control beside the title, expands an empty mailbox workspace to the available height without outer gutters, collapses mailbox controls behind a labeled toggle, gives search the full visible heading width, and presents edge-to-edge message rows with the subject above the sender. A message received today shows its local time, while an older message shows its day and abbreviated month.
...
| Trashed messages | `GET /v1/inbucket/trash/messages` with optional `cursor`, `search`, `read`, `mailbox`, `sort`, and `limit` | `200` current user's filtered summaries with availability, exact filtered and total Trash counts, mailbox options, and nullable next cursor | `401` expired, `422` invalid query or cursor, other status visible load error |
| Move or restore message | `PATCH /v1/inbucket/mailboxes/:name/messages/:id/trashed` with boolean `trashed` JSON | `200` trashed state and summary or availability | `401` expired, `404` missing indexed message, `422` invalid value |
| Empty trash | `DELETE /v1/inbucket/trash` | `200` one result per trashed message; successful upstream deletions remove local derived data and failures remain | `401` expired, per-message upstream failures are reported in the result array |
...
| Delete message permanently | `DELETE /v1/inbucket/message?name=...&id=...` | `204`, including removal of local derived data after upstream success | `401` expired, `404` missing, `502` upstream unavailable without local cleanup |
