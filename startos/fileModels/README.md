Use the `/fileModels` directory to create separate `.ts` files to represent underlying files used by your package. The exported `FileModels` afford a convenient and type-safe way to read and write the underlying files, as well as react to changes.
